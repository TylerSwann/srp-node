...
sjcl.cipher.aes = function (a) {
    this.M[0][0][0] || this.T();
    var b, d, c, e, f = this.M[0][4], g = this.M[1];
    b = a.length;
    var h = 1;
...
sjcl.cipher.aes.prototype = {
    encrypt: function (a) {
        ...
        var a = this.M[0], b = this.M[1], d = a[4], c = b[4], e, f, g, h = [], k = [], l, m, n, p;
...
sjcl.codec.base32 = {
    ...
    fromBits: function (a, b, d) {
        var c = sjcl.codec.base32.BASE, e = sjcl.codec.base32.REMAINING, f = "", g = 0, h = sjcl.codec.base32.D, k = 0,
    ...
    toBits: function (a, b) {
        ...
        var d = sjcl.codec.base32.BITS,
            c = sjcl.codec.base32.BASE, e = sjcl.codec.base32.REMAINING, f = [], g, h = 0, k = sjcl.codec.base32.D,
            l = 0, m, n = "base32";
...
sjcl.codec.base64 = {
    D: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", fromBits: function (a, b, d) {
        var c = "", e = 0, f = sjcl.codec.base64.D, g = 0, h = sjcl.bitArray.bitLength(a);
        d && (f = f.substr(0, 62) + "-_");
        ...
        a = a.replace(/\s|=/g, "");
        var d = [], c, e = 0, f = sjcl.codec.base64.D, g = 0, h;
        b && (f = f.substr(0, 62) + "-_");
...
sjcl.hash.sha256.prototype = {
    blockSize: 512, reset: function () {
        ...
        var b, d, c, e = this.c, f = this.i, g = e[0], h = e[1], k = e[2], l = e[3], m = e[4], n = e[5], p = e[6],
            r = e[7];
...
sjcl.hash.sha512.prototype = {
    ...
    m: function (a) {
        var b,
            d, c = this.c, e = this.i, f = c[0], g = c[1], h = c[2], k = c[3], l = c[4], m = c[5], n = c[6], p = c[7],
            r = c[8], t = c[9], D = c[10], I = c[11], x = c[12], B = c[13], A = c[14], y = c[15], u;
        if ("undefined" !== typeof Uint32Array) {
            u = Array(160);
            for (var v = 0; 32 > v; v++) u[v] = a[v]
        } else u = a;
        var v = f, q = g, w = h, J = k, L = l, K = m, X = n, M = p, E = r, C = t, T = D, N = I, U = x, O = B, Y = A,
            P = y;
        for (a = 0; 80 > a; a++) {
            if (16 > a) b = u[2 * a], d = u[2 * a + 1]; else {
                ...
                var G = u[2 * (a - 2) + 1], z =
                    ...
                    G = (d << 13 | G >>> 19) ^ (G << 3 | d >>> 29) ^ (d << 26 | G >>> 6), Z = u[2 * (a - 7)],
                    aa = u[2 * (a - 16)], Q = u[2 * (a - 16) + 1];
                d = F + u[2 * (a - 7) + 1];
                b = b + Z + (d >>> 0 < F >>> 0 ? 1 : 0);
                d += G;
                b += z + (d >>> 0 < G >>> 0 ? 1 : 0);
            ...
            u[2 * a] = b |= 0;
            u[2 * a + 1] = d |= 0;
            var Z = E & T ^ ~E & U, ga = C & N ^ ~C & O, G = v & w ^ v & L ^ w & L, ka = q & J ^ q & K ^ J & K,
                aa = (q << 4 | v >>> 28) ^ (v << 30 | q >>> 2) ^ (v << 25 | q >>> 7),
                Q = (v << 4 | q >>> 28) ^ (q << 30 | v >>> 2) ^ (q << 25 | v >>> 7), la = e[2 * a], ha = e[2 * a + 1],
                z = P + ((E << 18 | C >>> 14) ^ (E << 14 | C >>> 18) ^ (C << 23 | E >>> 9)),
                F = Y + ((C << 18 | E >>> 14) ^ (C << 14 | E >>> 18) ^ (E <<
                    23 | C >>> 9)) + (z >>> 0 < P >>> 0 ? 1 : 0), z = z + ga,
                F = F + (Z + (z >>> 0 < ga >>> 0 ? 1 : 0)), z = z + ha, F = F + (la + (z >>> 0 < ha >>> 0 ? 1 : 0)),
                z = z + d | 0, F = F + (b + (z >>> 0 < d >>> 0 ? 1 : 0));
            d = Q + ka;
            b = aa + G + (d >>> 0 < Q >>> 0 ? 1 : 0);
            Y = U;
            P = O;
...
sjcl.hash.sha1.prototype = {
    ...
    i: [1518500249, 1859775393, 2400959708, 3395469782], m: function (a) {
...
sjcl.mode.ccm = {
    name: "ccm", W: [], listenProgress: function (a) {
        ...
        e = e || 64;
        c = c || [];
        var f = sjcl.bitArray, g = f.bitLength(d) / 8, h = f.bitLength(b), k = f.clamp(b, h - e),
...
sjcl.beware["CTR mode is dangerous because it doesn't protect message integrity."] = function () {
    sjcl.mode.ctr = {
        name: "ctr", encrypt: function (a, b, d, c) {
            ...
            for (g = 0; g < c; g += 4) for (f = a.encrypt(d), e[g] ^= f[0], e[g + 1] ^= f[1], e[g + 2] ^= f[2], e[g + 3] ^= f[3], f = 3; 0 <= f && !++d[f]; f--) ;
...
sjcl.beware["CBC mode is dangerous because it doesn't protect message integrity."] = function () {
    sjcl.mode.cbc = {
        name: "cbc", encrypt: function (a, b, d, c) {
...
sjcl.mode.ocb2 = {
    name: "ocb2", encrypt: function (a, b, d, c, e, f) {
        ...
        if (128 !== sjcl.bitArray.bitLength(d)) throw new sjcl.exception.invalid("ocb iv must be 128 bits");
        e = e || 64;
        var g = sjcl.mode.ocb2.U, h = sjcl.bitArray, k = h.l, l = [0, 0, 0, 0], m = g(a.encrypt(d)), n, p,
            r = sjcl.bitArray.bitLength(b) - e, t = [];
        c = c || [];
        ...
        var d, c = sjcl.mode.ocb2.U, e = sjcl.bitArray, f = e.l, g = [0, 0, 0, 0], h = a.encrypt([0, 0, 0, 0]),
            h = f(h, c(c(h)));
...
sjcl.mode.gcm = {
    name: "gcm", encrypt: function (a, b, d, c, e) {
        var f = b.slice(0);
        b = sjcl.bitArray;
        c = c || [];
...
sjcl.misc.hmac.prototype.digest = function () {
    var a = this.ea.finalize(), a = (new this.ka(this.P[1])).update(a).finalize();
    this.reset();
    return a
};
...
sjcl.misc.scrypt.reverse = function (a) {
    for (var b in a) {
        var d = a[b] & 255, d = d << 8 | a[b] >>> 8 & 255, d = d << 8 | a[b] >>> 16 & 255,
            d = d << 8 | a[b] >>> 24 & 255;
        a[b] = d
    }
};
        ...
        switch (typeof a) {
            ...
            case "object":
                ...
                if ("[object Uint32Array]" === d) {
                    ...
                    a = e
                } else for ("[object Array]" !== d && (k = 1), d = 0; d < a.length && !k; d++) "number" !== typeof a[d] &&
                (k = 1);
                if (!k) {
...
a:try {
    ...
    if (ia && fa.randomBytes) S = fa.randomBytes(128), S = new Uint32Array((new Uint8Array(S)).buffer), sjcl.random.addEntropy(S, 1024, "crypto['randomBytes']"); else if ("undefined" !== typeof window && "undefined" !== typeof Uint32Array) {
        ...
        if (window.crypto && window.crypto.getRandomValues) window.crypto.getRandomValues(V); else if (window.msCrypto && window.msCrypto.getRandomValues) window.msCrypto.getRandomValues(V);
        else break a;
...
sjcl.json = {
    defaults: {v: 1, iter: 1E4, ks: 128, ts: 64, mode: "ccm", adata: "", cipher: "aes"}, Ca: function (a, b, d, c) {
        d = d || {};
        c = c || {};
        ...
        var e = sjcl.json, f = e.Ca.apply(e, arguments);
        return e.encode(f)
    }, Ba: function (a, b, d, c) {
        d = d || {};
        c = c || {};
...
sjcl.bn.prototype = {
    radix: 24, maxMul: 8, o: sjcl.bn, copy: function () {
    ...
    trim: function () {
        ...
        var b = Math.floor((a - 1) / 24), d = sjcl.bitArray,
...
sjcl.bn.fromBits = function (a) {
    var b = new this, d = [], c = sjcl.bitArray, e = this.prototype,
        f = Math.min(this.bitLength || 0x100000000, c.bitLength(a)), g = f % e.radix || e.radix;
...
sjcl.bn.pseudoMersennePrime = function (a, b) {
    ...
    var c = d.prototype = new sjcl.bn, e, f, g;
    ...
    c.reduce = function () {
        var a, b, d, c = this.modOffset, e = this.limbs, f = this.offset, g = this.offset.length, t = this.factor, D;
...
sjcl.ecc.point.prototype = {
    toJac: function () {
        ...
        if (void 0 === this.pa) {
            d = this.toJac().doubl();
            a = [d];
            for (b = 3; 16 > b; b++) d = d.add(this), a.push(d);
            this.pa = [new sjcl.ecc.point(this.curve), this].concat(sjcl.ecc.pointJac.toAffineMultiple(a))
        }
        return this.pa
    }, negate: function () {
        var a =
...
sjcl.ecc.pointJac.toAffineMultiple = function (a) {
    for (var b = 0, d, c = Array(a.length), e, f, g, h, k; b < a.length; b++) {
        e = a[b];
        if (h != e.curve) {
            if (h) {
                for (b = 0; b < a.length; b++) c[b] = a[b].toAffine();
                return c
            }
            h = e.curve
        }
        e.isIdentity || e.z.equals(0) || (f ? (f.push(g), g = g.mul(e.z)) : (g = e.z, f = []))
    }
    f && (g = g.inverse(), d = f.length - 1);
    for (b--; 0 <= b; b--) e = a[b], e.isIdentity || e.z.equals(0) ? c[b] = new sjcl.ecc.point(e.curve) : (0 <= d ? (h = g.mul(f[d]), g = g.mul(e.z), d--) : h = g, k = h.square(), c[b] = new sjcl.ecc.point(e.curve, e.x.mul(k).fullReduce(), e.y.mul(k.mul(h)).fullReduce()));
    return c
};
sjcl.ecc.pointJac.prototype = {
    add: function (a) {
        ...
        var a = this.y.square(), b = a.mul(this.x.mul(4)), d = a.square().mul(8), a = this.z.square(),
            ...
            a = c.square().subM(b).subM(b), b = b.sub(a).mul(c).subM(d), d = this.y.add(this.y).mul(this.z);
        return new sjcl.ecc.pointJac(this.curve, a, b, d)
    }, toAffine: function () {
        ...
        for (d = a.length - 1; 0 <= d; d--) for (c = sjcl.bn.prototype.radix - 4; 0 <= c; c -= 4) e =
...
sjcl.ecc.basicKey = {
    publicKey: function (a, b) {
        ...
        this.serialize = function () {
            ...
            return {type: this.getType(), secretKey: !1, point: sjcl.codec.hex.fromBits(this.H.toBits()), curve: b}
        };
        this.get = function () {
            var a = this.H.toBits(), b = sjcl.bitArray.bitLength(a), e = sjcl.bitArray.bitSlice(a, 0, b / 2),
...
sjcl.ecc.ecdsa.publicKey.prototype = {
    verify: function (a, b, d) {
        ...
        var c = sjcl.bitArray, e = this.w.r, f = this.I, g = sjcl.bn.fromBits(c.bitSlice(b, 0, f)),
            c = sjcl.bn.fromBits(c.bitSlice(b, f, 2 * f)), h = d ? c : c.inverseMod(e),
            f = sjcl.bn.fromBits(a).mul(h).mod(e), h = g.mul(h).mod(e), f = this.w.G.mult2(f, h, this.H).x;
...
sjcl.keyexchange.srp = {
    ...
    }, ma: [1024, 1536, 2048, 3072, 0x1000, 6144, 8192], na: {
        1024: {
...
sjcl.arrayBuffer.ccm = {
    mode: "ccm", defaults: {tlen: 128}, compat_encrypt: function (a, b, d, c, e) {
        ...
        d[3]++;
        0 === d[3] && d[2]++;
        if (0 !== b.byteLength) for (e = new DataView(b), m = 0; m < e.byteLength; m += 16) m > n && (sjcl.mode.ccm.ha(m / b.byteLength), n += p), l = a.encrypt(d),
...
(function () {
    ...
    sjcl.hash.ripemd160.prototype = {
        reset: function () {
            ...
            var a = sjcl.bitArray.concat(this.h, [sjcl.bitArray.partial(1, 1)]), c = (this.f + 1) % 512,
                c = (448 < c ? 512 : 448) - c % 448, e = c % 32;
